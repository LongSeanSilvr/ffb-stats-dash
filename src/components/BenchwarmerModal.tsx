...
export const BenchwarmerModal: React.FC<Props> = ({ matchup, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-surface-color border border-white/10 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="bg-black/40 border-b border-white/5 p-4 md:p-6 flex items-center justify-between">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h2 className="text-lg md:text-xl font-bold text-white leading-none mb-1">Benchwarmer Blues: Week {matchup.week}</h2>
            ...
            <div>
              ...
              <div className="space-y-2">
                {matchup.actualStarters.map((s, idx) => {
                  ...
                  return (
                    <div key={idx} className={`flex items-center justify-between p-2 md:p-3 rounded-lg border ${isBenchedInOptimal ? 'bg-danger-color/10 border-danger-color/30' : 'bg-white/20 border-white/5'}`}>
                      <div className="flex items-center gap-2 md:gap-3 overflow-hidden">
                        ...
                        <span className={`text-xs md:text-sm font-medium truncate ${isBenchedInOptimal ? 'text-danger-color font-bold' : 'text-white'}`}>{s.name}</span>
                      </div>
