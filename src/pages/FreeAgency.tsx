...
const MatrixTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div style={{ background: 'rgba(15,17,21,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '1rem', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
      ...
      <div className="text-sm text-muted">Avg Hold Time: <span className="text-accent-color font-bold ml-1">{d.averageWeeksHeld} weeks</span></div>
      <div className="text-sm text-muted mt-2 border-t border-white/10 pt-2">Hit Probability: <span className="text-success-color font-bold ml-1">{d.hitRate}%</span></div>
    </div>
  );
};
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <Card className="stagger-1">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-base font-semibold">Streamer vs. Stasher Matrix</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <Card className="stagger-2">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-base font-semibold">Waiver Impact: Wins Created</h2>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={hitData} layout="vertical" margin={{ left: 60, bottom: 20 }}>
                ...
                <RechartsTooltip cursor={{ fill: 'rgba(255,255,255,0.03)' }} contentStyle={{ backgroundColor: 'rgba(15,17,21,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px' }}
                  formatter={(value: any) => [`${value} Matchups`, 'Impact']} />
                <Bar dataKey="waiverWins" fill="var(--accent-color)" radius={[0, 4, 4, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      {/* Grid 3: Transaction Urgency Heatmap */}
      <Card className="stagger-3 mb-8">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-base font-semibold mb-1">Transaction Timing (Urgency) Heatmap</h2>
            <p className="text-xs text-muted">Density of waiver and free agency claims by day of the week.</p>
          </div>
          <ChartToggle value={hitFilter} onChange={setHitFilter} />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse table-fixed min-w-[600px]">
            <thead>
              <tr>
                <th className="py-2 px-3 text-xs font-semibold text-muted border-b border-white/10 w-48">Manager</th>
                {['Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun', 'Mon'].map(d => (
                  <th key={d} className="py-2 px-3 text-xs font-semibold text-muted text-center border-b border-white/10">{d}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {hitData.map(mgr => {
                const maxVal = Math.max(...Object.values(mgr.transactionsByDay || {}));
                return (
                  <tr key={mgr.name} className="border-b border-white/[0.02] transition-colors hover:bg-white/[0.02]">
                    <td className="py-3 px-3">
                      <div className="flex items-center text-sm font-medium">
                        {mgr.avatar 
                          ? <img src={`https://sleepercdn.com/avatars/thumbs/${mgr.avatar}`} style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, marginRight: '12px' }} /> 
                          : <div style={{ width: 28, height: 28, borderRadius: '50%', backgroundColor: '#475569', flexShrink: 0, marginRight: '12px' }} />
                        }
                    ...
                    {['Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun', 'Mon'].map(d => {
                      ...
                      const opacity = maxVal > 0 ? 0.2 + (val / maxVal) * 0.8 : 0.1;
                      return (
                        <td key={d} className="py-2 px-1">
                          <div className="w-full h-8 rounded flex items-center justify-center mx-auto" style={{ backgroundColor: val > 0 ? `rgba(16, 185, 129, ${opacity})` : 'rgba(255,255,255,0.02)' }} title={`${val} transactions`}>
                             {val > 0 && <span className="text-[10px] text-white/70 font-semibold">{val}</span>}
                          </div>
                        </td>
                      );
                    })}
      ...
      <Card className="stagger-4 mb-10">
        <div className="flex justify-between items-start mb-4">
          <div>
